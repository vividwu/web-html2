...
${this.column?this.column.map(i => html`<th>${i.title}</th>`):''}
</tr></thead>
<tbody>
${this.data?this.data.map(d => html`<tr>${this.column?this.column.map(col => html`<td>${col.render?col.render(d):d[col.key]}</td>`):''}</tr>`):''}
