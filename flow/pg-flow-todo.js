...
import {WorkFlowTodoApi,WorkFlowCompleteApi,WorkFlowMyListApi} from '../webapi.js';
...
.nav-item a:hover {
  text-decoration: none; }
    ...
    approvedTabClick(){
        ...
        this._applyedTab = false;
        this.requestUpdate();
        WorkFlowCompleteApi({page_num:1,page_size:20}).then(res => {
    ...
    applyedTabClick(){
        ...
        this.requestUpdate();
        WorkFlowMyListApi({page_num:1,page_size:20}).then(res => {
            console.log('mylist',res);
        this._todoList = JSON.stringify(res.data.list);
        this.requestUpdate();
    });
    }
    tablePreRender(e){debugger
        let that = this;
            ...
            ,{labelName:'操作',id:'id',render:function(row){debugger
                    return html`<a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
									<span class="svg-icon svg-icon-md svg-icon-primary">
										...
										${that._applyedTab?that.viewIcon():that.editIcon()}
										<!--end::Svg Icon-->
									</span>
								</a>`;
                }
    ...
    viewIcon(){
        return html`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
