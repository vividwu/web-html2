...
(function(mod) {
  ...
  Completion.prototype = {
   ...
   /*   if (this.widget1)
      ...
      if (data && data.list.length) {
        // if (picked && data.list.length == 1) {只有一个匹配时不直接填充
        //   this.pick(data, 0);
        // } else {
  ...
  //api文档说明
  var objContent = {};
  objContent.DbOpenconnStr = ["开启数据库 DbOpen(string connStr) 说明：创建一个可访问的数据库实例，返回操作对象 返回值：db对象"];
  objContent.HttpOpenmethodurlheaderparamtimeout = ["开启网络 HttpOpen(string method,string url,hash header,string param,int timeout) 说明：method:GET/POST header:{\"Content-Type\":...} param:\"a=xx&b=yy\" [timeout]选填，默认5000ms 返回值：http对象"];
  objContent.printlnobj = ["打印对象字面量 println(object obj) 说明：系统打印，无返回值 返回值：void"];
  objContent.lenobj = ["获取列表或字符串长度 len(list obj) 说明：仅适用于列表/字符串对象，返回数值长度 返回值：int"];
  objContent.typeobj = ["获取对象类型 type(object obj) 说明：返回类型字符串，内置类型：INT/STRING/ARRAY/HASH/NULL/ERROR 返回值：string"];

  objContent.tostring=[
    "int类型转字符串 tostring() 说明：仅适int对象上调用 返回值：string",
    "字符串转int类型 toint() 说明：仅适string对象上调用 返回值：int",
    "字符串转int类型 split(string separater) 说明：仅适string对象上调用，返回字符串列表 返回值：array",
    "HttpOpen函数返回结果解析成字符串 string() 说明：仅适用于HttpOpen函数返回对象上调用 返回值：string",
    "HttpOpen函数返回结果解析成Json对象 json() 说明：仅适用于HttpOpen函数返回对象上调用 返回值：hash",
    "DbOpen返回结果行的列表 select(string sql,params) 说明：仅适用于DbOpen函数返回对象上调用，返回哈希列表，params可以传多个。eg:db.select(\"select*from user where id=? and name=?\",1,\"vivid\");a[0][\"filed_name\"] 返回值：array<hash>"];
        ...
        //for (var i = 0; i < 1; ++i) {
          ...
          if(typeof(key) != "undefined")
          {
            ...
            index = showList[0];
              ...
              key = (cur.displayText || getText(cur)).replaceAll(",","").replace("(","").replace(")","").replace(" ","");
          }
      ...
      try{
        ...
        if(typeof(key) != "undefined")
        {
          ...
          index = showList[i];
            ...
            key = this.data.list[0].replaceAll(",","").replace("(","").replace(",","").replace(")","").replace(" ","");
        }
